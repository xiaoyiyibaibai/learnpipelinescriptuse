        ...
        node {
          stage('prepare') {
             if (currentBuild.previousBuild&&currentBuild.previousBuild.result == null) {
              ...
              withEnv(['MYNAME=xiaodonghong,PATH+LIMEI=我是特殊信息,PATH+LIMEI2=/我是特殊信息2']) {
                        println("代码块中env.MYName自定义的环境变量是${env.MYName}")
                        println("代码块中MYName自定义的环境变量是${MYName}")
                        println("代码块中env.LIMEI自定义的环境变量是${env.LIMEI}")
                        println("代码块中LIMEI自定义的环境变量是${LIMEI}")
                        println("代码块中env.LIMEI2自定义的环境变量是${env.LIMEI2}")
                        println("代码块中LIMEI2自定义的环境变量是${LIMEI2}")
              }
